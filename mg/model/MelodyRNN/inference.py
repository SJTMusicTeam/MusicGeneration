...
net.load_state_dict(torch.load('./basic_rnn/basic_rnn_6679.pth'))
def predict_melody_rnn(model, num_gen, prefix, device):
    net.to(device)
    net.eval()
    state = (torch.tensor(np.random.rand(model.num_layers, 1, model.num_hiddens),dtype=torch.float32,device=device),
             torch.tensor(np.random.rand(model.num_layers, 1, model.num_hiddens),dtype=torch.float32,device=device))
    output = prefix
